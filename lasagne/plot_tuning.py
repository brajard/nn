...
corrapp = dict()
dcol = dict()
lcol = ['green','cyan','blue','red','black','magenta']
...
for ntype in net_type:
    ...
    for i,(p,m,app) in enumerate(lparam):
        ...
        corr[ntype][i]=m
        corrapp[ntype][i]=app
    plt.plot(nw[ntype],corr[ntype],'.',markersize=10,label=ntype,color=dcol[ntype])
    #print of the best
...
#Plot pour la AMS
    ...
    for ntype in lnet:
        plt.plot(nw[ntype],corr[ntype],'.',markersize=10,label=ntype,color=dcol[ntype],zorder=zorder)
        zorder = zorder -1
